...
def read_data(filepath="ml-1m/ratings.dat") -> pd.DataFrame:
    """
    Input: filepath
    Output: dataframe
    """
...
def get_user_item_matrix(data: pd.DataFrame) -> pd.DataFrame:
    """
    Input: dataframe
...
def get_similarity_matrix(matrix_norm: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    item_similarity_matrix_matrix = pd.DataFrame(
        item_similarity_matrix, index=matrix_norm.index, columns=matrix_norm.index
    )

    return item_similarity_matrix_matrix


def watched_movies(matrix_norm: pd.DataFrame, picked_userid: int) -> pd.DataFrame:
    """
    Output: a dataframe of movies that the target user has watched with two columns—movie_id, rating
            sorted by rating in descending order
    """

    # Movies that the target user has watched
    picked_userid_watched = pd.DataFrame(
        matrix_norm.loc[:, picked_userid]
...
def unwatched_movies(matrix_norm, picked_userid) -> list:
    """
    ...
    picked_userid_unwatched = picked_userid_unwatched[
        picked_userid_unwatched[picked_userid] == True
    ].index.tolist()
...
def get_predicted_rating(
    ...
    picked_movie_similarity_score = item_similarity_matrix[[picked_movie_id]].rename(
        columns={"index": "movie_id", picked_movie_id: "similarity_score"}
    )
...
def predict_ratings(picked_userid, movie_id, item_similarity_matrix, matrix_norm):
    ...
    predicted_rating = get_predicted_rating(
        movie_id, picked_userid_watched, item_similarity_matrix
    )
...
def predict_one_user(picked_userid, item_similarity_matrix, matrix_norm, mv_pct=0.05):
    ...
    for picked_movie_id in random_movieids:
        ...
        predicted_rating = predict_ratings(
            picked_userid, picked_movie_id, item_similarity_matrix, matrix_norm
        )
        # append the predicted rating to the dataframe
        predictions.append(
            {
                "user_id": picked_userid,
                "movie_id": picked_movie_id,
...
def generate_predicted_ratings(matrix_norm, item_similarity_matrix, user_pct=0.05):
    ...
    # randomly pick a portion of (user_pct) users
    picked_userids = np.random.choice(
        matrix_norm.columns, size=round(matrix_norm.shape[1] * user_pct), replace=False
    )
    ...
    for picked_userid in picked_userids:
        ...
        predictions = predict_one_user(
            picked_userid, item_similarity_matrix, matrix_norm
        )
...
def calculate_MAE_RMSE(predicted_ratings_df, data):
    ...
    predicted_ratings = pd.merge(
        left=predicted_ratings_df,
    ...
    MAE = np.mean(
        abs(predicted_ratings["predicted_rating"] - predicted_ratings["rating"])
    )
    RMSE = np.sqrt(
        np.mean(
            (predicted_ratings["predicted_rating"] - predicted_ratings["rating"]) ** 2
        )
    )
...
def recommend_top_n_to_user(picked_userid, top_n=10):
    ...
    recommendations = recommend_items(
        picked_userid, item_similarity_matrix, matrix_norm, top_n=top_n
    )
